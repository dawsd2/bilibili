...
public class RegisterRequest {
    ...
    public String web3(UserEntity user, HttpServletRequest request, String yanzheng) {
        ...
        //验证QQ
        String userQQ = "[1-9][0-9]{4,10}";
        Pattern uersqq = Pattern.compile(userQQ);
        Matcher matcherqq = uersqq.matcher(user.getUserQQ());
        boolean userq = matcherqq.matches();

        String sex = "^(男|女){1}$";
        Pattern usersex = Pattern.compile(sex);
        Matcher matchersex = usersex.matcher(user.getUsersex());
        boolean usex = matchersex.matches();
                ...
                if (userq) {
                    if (usex) {
                        if (emial) {
                            ...
                            request.setAttribute("fail", "注册失败,邮箱输入错误");
                            return "forward:/zhuce.sf";
                        }
                    } else {
                        request.setAttribute("fail", "注册失败，请选择性别");
                        return "forward:/zhuce.sf";
                    }
                } else {
                    request.setAttribute("fail", "注册失败，请输入正确的QQ");
                    return "forward:/zhuce.sf";
                }
            } else {
